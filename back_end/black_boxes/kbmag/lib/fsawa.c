 ...
 * It is an alternative to fsa_nosub_exists, which works on the output
 * of a word-difference machine for the group "anded" with the "greater-than"
 * automataon.
 ...
 * (w_1,w_2) is accepted by *fsaptr and w_2 < w_1 in the shortlex ordering,
...
fsa_wa_short(fsa *fsaptr, storage_type op_table_type, boolean destroy, char *tempfilename)
{
  ...
  if (fsaptr->table->table_type!=DENSE) {
     fprintf(stderr,
      "Error: function fsawa can only be called with a densely-stored fsa.\n");
     exit(1);
  }
...
fsa *
fsa_wa_int(fsa *fsaptr, storage_type op_table_type, boolean destroy, char *tempfilename) // Changed table_struc to storage_type
{
